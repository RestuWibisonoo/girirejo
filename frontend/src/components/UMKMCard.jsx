...
const UMKMCard = ({ umkm }) => {
  ...
  return (
    <div className="bg-white rounded-2xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] border border-stone-100 overflow-hidden hover:shadow-xl hover:-translate-y-2 transition-all duration-300 flex flex-col group">
      {/* Foto Produk dan Detail (Clickable) */}
      <Link to={`/umkm/${umkm.id}`} className="flex flex-col flex-grow cursor-pointer">
        ...
        <div className="p-5 pb-0 flex flex-col flex-grow">
          <h3 className="font-extrabold text-slate-800 text-lg mb-2 leading-snug group-hover:text-brand-primary transition-colors">{umkm.nama_usaha}</h3>
          {umkm.deskripsi && (
      ...
      {/* Tombol Aksi */}
      <div className="px-5 pb-5 mt-4 grid grid-cols-1 xl:grid-cols-2 gap-2">
        {waLink ? (
          <a
            href={waLink}
            target="_blank"
